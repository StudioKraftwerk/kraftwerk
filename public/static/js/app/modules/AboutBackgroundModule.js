define([
    ...
    return Superhero.Module.extend({
        ...
        objects: {
            ...
            bolt: {
                path: '/static/three/icon-bolt.js',
                object: null,
                geom: null,
                geomBase: null,
                canMouseMoveTimer: null,
            ...
            hand: {
                path: '/static/three/icon-hand.js',
                object: null,
                geom: null,
                geomBase: null,
                canMouseMoveTimer: null,
        ...
        animateIconBoltIn : function(){
            this._animateIconIn(this.objects.bolt);
        },
        animateIconBoltOut : function(){
            this._animateIconOut(this.objects.bolt);
        },

        animateIconEyeIn : function(){
            this._animateIconIn(this.objects.eye);
        },
        animateIconEyeOut : function(){
            this._animateIconOut(this.objects.eye);
        },

        animateIconHandIn : function(){
            this._animateIconIn(this.objects.hand);
        },
        animateIconHandOut : function(){
            this._animateIconOut(this.objects.hand);
        },
        ...
        _load: function() {
            ...
            this.loader.load( this.objects.bolt.path, function(geometry) {
                this.objects.bolt.geomBase = geometry.clone();
                this.objects.bolt.geom = geometry;
                this._loadComplete();
            }.bind(this));

            this.loader.load( this.objects.eye.path, function(geometry) {
                this.objects.eye.geomBase = geometry.clone();
                this.objects.eye.geom = geometry;
                this._loadComplete();
            }.bind(this));

            this.loader.load( this.objects.hand.path, function(geometry) {
                this.objects.hand.geomBase = geometry.clone();
                this.objects.hand.geom = geometry;
                this._loadComplete();
            }.bind(this));
        },
        ...
        _loadCompleteAll: function() {
            ...
            this.objects.bolt.object = this._createObject(this.objects.bolt.geom);
            this.objects.eye.object = this._createObject(this.objects.eye.geom);
            this.objects.hand.object = this._createObject(this.objects.hand.geom);


            this.container = new THREE.Object3D();
            this.container.add(this.objects.bolt.object);
            this.container.add(this.objects.eye.object);
            this.container.add(this.objects.hand.object);
        ...
        _setupEventListeners: function() {
            ...
            this.listenTo(ResizeManager, 'resize:complete', this._handleResize);
        ...
        _createObject: function(geometry) {
            var object = new THREE.Object3D();
        ...
        _animateIconIn: function(icon) {

            icon.object.visible = true;

            icon.canMouseMove = false;
            icon.canMouseMoveTimer = setTimeout(function() {
                icon.canMouseMove = true;
            }, 700);

            TweenMax.to(icon.object.position, 0.3, {
                z: 2,
                ease: Expo.easeIn,
                overwrite: "all"
            });
            ...
            var v = 0;
            var speed = 0;
            for(j=0; j<icon.geom.vertices.length; j++) {
                v = icon.geom.vertices[j];
                vBase = icon.geomBase.vertices[j];

                speed = 0.4;
                TweenMax.fromTo(v, speed, {
                    x: vBase.x + (Math.random()*20-10),
                    y: vBase.y + (Math.random()*20-10),
                    z: vBase.z - 5
                },
                {
        ...
        _animateIconOut: function(icon) {

            icon.canMouseMove = false;
            if(icon.canMouseMoveTimer) clearTimeout(icon.canMouseMoveTimer);

            TweenMax.to(icon.object.position, 0.15, {
                z: -2,
                ease: Expo.easeIn,
                overwrite: "all",
                onComplete: function() {
                    icon.object.visible = false;
                },
            });
        ...
        _updateCamera: function() {
            ...
            this.container.rotation.x += (targetRotation.x - this.container.rotation.x) * easing;
            this.container.rotation.y += (targetRotation.y - this.container.rotation.y) * easing;

            this.container.position.x = this.mouse.x/1000;
            this.container.position.y = this.mouse.y/1000;
        },
